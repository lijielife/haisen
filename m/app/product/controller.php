...
class ProductController extends Controller 
{
    ...
    // 页面查看次数统计
    public function stats_view_goods_count( $id = 0, $return = false ) {
        if ( $id <= 0 ) return 0;
    ...
    // 根据商品id获取评论数量
    public function get_comment_count( $gid = 0 ) {
        $g = "";
        if ( $gid > 0 ) 
        {
            $g = "AND id_value = '$gid'";
    ...
    // 根据商品id获取评论信息
    public function get_comment_list($gid=0,$start=0,$list=10){
        //以下是以用户作为条件查询的评论
    ...
    public function index( $gid = 0 ) 
    {
        ...
        if ( ! empty( $_COOKIE['HISTORYVIEW'] ) )
        {
            ...
            setcookie('HISTORYVIEW', implode(',', $history), time() + 3600 * 24 * 30);
        }
        else
        {
            setcookie('HISTORYVIEW', $gid, time() + 3600 * 24 * 30);
        }
        /* END 记录浏览历史 */
        ...
        if ( is_null( $rt ) ) {
            ...
            if($rt['goodsinfo']['is_promote']=='1' && $rt['goodsinfo']['promote_start_date'] < time() && $rt['goodsinfo']['promote_end_date'] > time()){$rt['goodsinfo']['is_promote']=='1';}else{$rt['goodsinfo']['is_promote']=='0';}
            if($rt['goodsinfo']['is_qianggou']=='1' && $rt['goodsinfo']['qianggou_start_date'] < time() && $rt['goodsinfo']['qianggou_end_date'] > time()){$rt['goodsinfo']['is_qianggou']=='1';}else{$rt['goodsinfo']['is_qianggou']=='0';}
            ...
            if ( $rt['goodsinfo']['is_promote'] == '1' ) {
                // 促销 价格
                if ( $rt['goodsinfo']['promote_start_date'] < time()&&$rt['goodsinfo']['promote_end_date']>time() ) {
            ...
            if(!empty($keys)){
                ...
                if(!empty($relategoods)){
                    foreach($relategoods as $k=>$row){
                        ...
                        if($row['is_promote']=='1'){
                            //促销 价格
                            if($row['promote_start_date']<time()&&$row['promote_end_date']>time()){
...
/*          $uid = $this->Session->read('User.uid');
            ...
            // 检查是否已经存在购买商品
            $w = "WHERE tb1.pay_status = 1 AND tb1.order_status=2 AND tb1.user_id=$uid ";
            $w .= "AND tb2.goods_id = ".$rt['goodsinfo']['goods_id'];
            $sql = "SELECT COUNT(*) FROM `{$this->App->prefix()}goods_order_info` AS tb1"; 
            $sql .= " LEFT JOIN `{$this->App->prefix()}goods_order` AS tb2 ON tb1.order_id = tb2.order_id";
            $sql .= " $w";
            $rt['order_count'] = $this->App->findvar( $sql );
            
            //检查该商品是否已经评论过
            $sql = "SELECT COUNT(*) FROM `{$this->App->prefix()}comment` WHERE id_value=$gid AND user_id=$uid";
            //$rt['comment_count'] = $this->App->findvar($sql)-$rt['order_count']+1; //评论2次
            $rt['comment_count'] = $this->App->findvar( $sql ) - $rt['order_count'];
        ...
        // 获取积分
        //$pointnum    = $rt['config']['pointnum'];
        //$pointnum_ag = $rt['config']['pointnum_ag'];
        //$scores      = intval($rt['goodsinfo']['pifa_price'] * $pointnum * $pointnum_ag);

        $this->set( 'scores', $rt['goodsinfo']['need_jifen'] );
    ...
    function ajax_comment($data=array(),$page=0){
        ...
        $json = Import::json();
                ...
                if(!($page>0)){
                    ...
                    if(!empty($read_time)){
                        if((time()-$read_time)<200){
                            $result['error'] = 3;
                            $result['message'] = '你刚才已经发表了评论，请你稍等下再发表！';
                            die($json->encode($result));
                        }
                    }
                    $this->Session->write("Comment.{$goods_id}",time());
    ...
    public function ajax_getcommentlist($data=array()){
    ...
    public function ajax_categoods_list( $page=0, $cid=0 ) {
