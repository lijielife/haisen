...
class CommonController extends Controller
{
	...
	$fn = $cache->fpath( array() );
        ...
        if ( file_exists( $fn ) && (time() - filemtime($fn) < 7000) && $refresh == false )
        {
            include( $fn );
        }
        else
	...
	file_put_contents("/wwwroot/custom_fenxiao/wxpay.log",date("Y-m-d H:i:s",time()).$this->curlGet($sURL).PHP_EOL,FILE_APPEND);
    ...
    function get_share_user_info()
    {
        ...
        if($issubscribe!='1' && !empty($openid)){ //获取分享者信息
            ...
            if($issubscribe=='1'){
                ...
                if(!empty($to_wecha_id)){
                    setcookie(CFGH.'USER[TOOPENID]', '', time() - 2592000);
                }
                ...
                if($issubscribe!='1'){
                    setcookie(CFGH.'USER[SUBSCRIBE]', $issubscribe, time() + 2592000);
                }
            }
            return array();
        }
    ...
    function get_user_wecha_id_info( $wecha_id = '', $ts = '0' )
    {
        ...
        if ( file_exists($fn)&& (time() - filemtime($fn) < 7000) && !$cache->GetClose() && $ts=='0' )
        {
            include( $fn );
        }
        else
    ...
    function get_user_code()
    {
        ...
        if ( $tid > 0 )
        {
            setcookie(CFGH.'USER[TID]', $tid, time() + 2592000);
            $this->Session->write('User.tid',$tid);
        }
        if ( $toid > 0 )
        {
            $this->Session->write('User.to_wecha_id',$toid); //来源ID
            setcookie(CFGH.'USER[TOOPENID]', $toid, time() + 2592000);
        }
                
        $this->Session->write('User.codetime',time());
        setcookie(CFGH.'USER[CODETIME]', time(), time() + 2592000);
        
        $thisurl = Import::basic()->thisurl();
        $this->Session->write('User.url',$thisurl); //记录当前进入连接
        setcookie(CFGH.'USER[URL]', $thisurl, time() + 2592000);
    ...
    function checkjump() 
    {
        ...
        if ( $issubscribe!='1' ) {
            ...
            if ( $issubscribe != '1' ) {
                ...
                setcookie(CFGH.'USER[SUBSCRIBE]', '0', time() + 2592000);
                ...
                if ( $to_wecha_id > 0 ) {
                    ...
                    setcookie(CFGH.'USER[TOOPENID]', $to_wecha_id, time() + 2592000);
                }
            }
        } 
...
/*      if($GLOBALS['LANG']['is_cache']=='1'&&!isset($_GET['code'])){
            ...
            setcookie(CFGH.'USER[TOOPENID]', "", time()-3600);
            setcookie(CFGH.'USER[UKEY]', "", time()-3600);
            setcookie(CFGH.'USER[PASS]', "", time()-3600);
            setcookie(CFGH.'USER[TID]', "", time()-3600);
            setcookie(CFGH.'USER[CODETIME]', "", time()-3600);
            setcookie(CFGH.'USER[ISOAUTH]', "", time()-3600);
            setcookie(CFGH.'USER[APPID]', "", time()-3600);
            setcookie(CFGH.'USER[APPSECRET]', "", time()-3600);
            die('这是测试阶段，缓存已经清空完成....');
        }*/
        //授权判断
        ...
        if ( empty($appid) || empty($appsecret) ) {
            ...
            setcookie(CFGH.'USER[ISOAUTH]', $rt['is_oauth'], time() + 3600*24);
        }
		...
		file_put_contents("/wwwroot/custom_fenxiao/userinfo.log",date("Y-m-d H:i:s",time())."common access_token: ".$access_token.PHP_EOL,FILE_APPEND);
		file_put_contents("/wwwroot/custom_fenxiao/userinfo.log","oauth2.0 access_token: ".$con.PHP_EOL,FILE_APPEND);
			 ...
			 file_put_contents("/wwwroot/custom_fenxiao/userinfo.log","ACCESS_TOKEN_NULL!".PHP_EOL,FILE_APPEND);
                ...
                if ( ! empty( $access_token ) ) {
                    ...
                    setcookie(CFGH.'USER[UKEY]', $wecha_id, time() + 2592000);
			...
			file_put_contents("/wwwroot/custom_fenxiao/userinfo.log","WX_ID: ".$wecha_id.PHP_EOL."USERINFO:  ".$con.PHP_EOL,FILE_APPEND);

                    if(strpos($con,"errcode")){
                        $access_token = $this->_get_access_token(true);
                        $con = $this->curlGet($url);
                        file_put_contents("/wwwroot/custom_fenxiao/userinfo.log","TRY AGAIN! USERINFO:  ".$con.PHP_EOL.PHP_EOL,FILE_APPEND);
                    }
                    ...
                    if ( ! empty($con) ) {
                        ...
                        if ( $subscribe == '1' ) {
                            $json->nickname = json_encode( $json->nickname);
                            $json->nickname = preg_replace( "#(\\\ue[0-9a-f]{3})#ie", "", $json->nickname);
                            $json->nickname = preg_replace( "#(\\\ud[0-9a-f]{3})#ie", "", $json->nickname);
                            $json->nickname = json_decode( $json->nickname);
                            ...
                            if ( ! empty($wecha_id) ) {
                                ...
                                if ( $uid > 0 ) {
                                    ...
                                    if ( ! empty( $ddl ) ) {
                                        ...
                                        setcookie(CFGH.'USER[UID]', $uid, time() + 2592000);
                                    }
                                }
                            } else {
                                // 写入日记，获取openid为空
        ...
        if ( $_GET['tid'] ) {
            ...
            setcookie(CFGH.'USER[TID]', '', time() - 2592000);
        }
...
/*      if($uid > 0){
        ...
        if ( $uid > 0 ) {           
            ...
            if ( $uid > 0 ) {
                ...
                $datas['user_name'] = ! empty( $wecha_id ) ? $wecha_id : 'GZSH'.$tid.time();
                ...
                if ( $this->App->insert( 'user', $datas ) ) { //添加账户
                    ...
                    if ( $tid != $uid ) { // 加入分享表
                        ...
                        $dd['url']       = $thisurl;
                        $dd['addtime']   = time();
                        file_put_contents("user_tuijian_log.txt", var_export( $dd, true ).PHP_EOL, FILE_APPEND);
                    ...
                    //写入cookie
                    setcookie(CFGH.'USER[UKEY]', $datas['wecha_id'], time() + 2592000);
                    setcookie(CFGH.'USER[UID]', $uid, time() + 2592000);
    ...
    */
    function ajax_popbox($boxname="",$data=array()){
        if($data['type']=='cart'){
            ...
            if($rt['is_promote']=='1'){
                //促销 价格
                if($rt['promote_start_date']<time()&&$rt['promote_end_date']>time()){
            ...
            if($rt['is_qianggou']=='1'){
                //促销 价格
                if($rt['qianggou_start_date']<time()&&$rt['qianggou_end_date']>time()){
