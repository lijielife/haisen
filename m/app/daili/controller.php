...
class DailiController extends Controller
{
    ...
    //修改密码
    public function update_user_pass( $data = array() )
    {
        $newpass           = $data['pass'];
    ...
    // 修改提款密码
    public function update_user_passpay( $data = array() )
    {
    ...
    // 修改提款信息
    public function update_user_bank( $data = array() )
    {
    ...
    public function ajax_open_dailiapply($data=array()){
        $var = $data['ty'];
        $uid = $this->Session->read('User.uid');
        $this->App->update('user',array('is_dailiapply'=>$var),'user_id',$uid);
    }
    
    public function is_daili()
    {
        /*$uid = $this->Session->read('User.uid');
        ...
        if($rank=='1'){
                ...
                if($rank=='1' || $is_apply =='1'){
                    $this->jump(ADMIN_URL.'user.php',0,'你没有权限访问'); exit;
                }
    ...
    //
    public function ajax_moneyrank_page($rts=array()){
        $hh = $rts['hh'];
        $tops = $rts['tops'];
        $tops = intval($tops);
        if(($tops-$hh) >= 0){
    ...
    public function gonggao()
    {
        ...
        $this->title("资讯专区".' - '.$GLOBALS['LANG']['site_name']);
        $this->is_daili();
        ...
        if(!defined(NAVNAME)) define('NAVNAME', "资讯专区");
        $this->set('rt',$rt);
    ...
    public function v2_mymoney()
    {
        ...
        $sql = "SELECT
                ...
                LEFT JOIN `{$this->App->prefix()}goods_order_info` AS tb2 
                ON tb2.order_sn = tb1.order_sn
                WHERE
                    tb1.uid = '$uid'
                AND tb2.pay_status = '1'
                AND tb2.order_status = '2'
                AND tb2.shipping_status = '2'
                AND tb1.money > 0
                LIMIT 1";
        ...
        $sql = "SELECT
                ...
                FROM
                    `{$this->App->prefix()}user_money_change_cache` AS tb1
                LEFT JOIN `{$this->App->prefix()}group_goods_order_info` AS tb2 
                ON tb2.order_sn = tb1.order_sn
                WHERE
                    tb1.uid = '$uid'
                AND tb1.type = 'group'
                ...
                AND tb1.money > 0
                LIMIT 1";
        $rt['not_received_group'] = $this->App->findvar( $sql );
        
        /* 已收货团购订单 */
        $sql = "SELECT
                    SUM(tb1.money)
                FROM
                    `{$this->App->prefix()}user_money_change` AS tb1
                LEFT JOIN `{$this->App->prefix()}group_goods_order_info` AS tb2 
                ON tb2.order_sn = tb1.order_sn
                WHERE
                    tb1.uid = '$uid'
                AND tb2.shipping_status = '5'
                AND tb1.money > 0
                LIMIT 1";
        $rt['received_group'] = $this->App->findvar( $sql );
     ...
     * 成为分销
     */
    public function my_is_daili()
    {
        $this->checked_login();
        $uid = $this->Session->read('User.uid');
        
        $sql = "SELECT tb1.*,tb2.subscribe_time,tb2.reg_time,tb2.nickname,tb2.headimgurl,tb2.money_ucount,tb2.points_ucount,tb2.mypoints,tb2.share_ucount,tb2.guanzhu_ucount,tb2.is_subscribe FROM `{$this->App->prefix()}user_tuijian` AS tb1";
        $sql .=" LEFT JOIN `{$this->App->prefix()}user` AS tb2 ON tb1.uid = tb2.user_id";
        if ( $nickname = trim( $_GET['nickname'] ) )
        {
            $sql .=" WHERE tb1.parent_uid = '$uid' AND tb2.nickname LIKE '%{$nickname}%' AND tb2.user_rank != '1' AND tb1.uid!='$uid' ORDER BY tb2.user_id ASC";
        }
        else
        {
        ...
        defined( NAVNAME ) or define( 'NAVNAME', '成为分销' );
        $this->set( 'rt', $rt );
        $mb = $GLOBALS['LANG']['mubanid'] > 0 ? $GLOBALS['LANG']['mubanid'] : '';
        $this->template( $mb . '/v2_myuser' );
    }
    
    public function monrydeial_all()
    {
        $this->checked_login();
        $uid = 0;
        $sql = "SELECT * FROM `{$this->App->prefix()}user` WHERE user_id ='{$uid}' AND active='1' LIMIT 1";
        ...
        //一级人数
        $rt['zcount1'] = $this->App->findvar($sql);

        $sql  = "SELECT SUM(mymoney) FROM `{$this->App->prefix()}user` AS tb0 ";
        $sql .= "LEFT JOIN `{$this->App->prefix()}user_tuijian` AS tb1 ON tb1.id = tb0.user_id ";
        $sql .= "WHERE tb1.parent_uid='$uid' LIMIT 1";
        
        $rt['mymoney1'] = $this->App->findvar($sql);
        
        //二级
        $sql = "SELECT COUNT(tb2.id) FROM `{$this->App->prefix()}user_tuijian` AS tb1";
        ...
        $sql  = "SELECT SUM(tb0.mymoney) FROM `{$this->App->prefix()}user` AS tb0 ";
        $sql .= "LEFT JOIN `{$this->App->prefix()}user_tuijian` AS tb1 ON tb1.parent_uid='$uid' ";
        $sql .= "LEFT JOIN `{$this->App->prefix()}user_tuijian` AS tb2 ON tb2.parent_uid = tb1.uid ";
        $sql .= "WHERE tb2.id = tb0.user_id AND tb2.uid IS NOT NULL  LIMIT 1";
                        
        $rt['mymoney2'] = $this->App->findvar($sql);        
        
        $sql = "SELECT COUNT(tb3.id) FROM `{$this->App->prefix()}user_tuijian` AS tb1";
        $sql .= " LEFT JOIN `{$this->App->prefix()}user_tuijian` AS tb2 ON tb2.parent_uid = tb1.uid ";
        ...
        $sql .= "LEFT JOIN `{$this->App->prefix()}user_tuijian` AS tb2 ON tb2.parent_uid = tb1.uid ";
        $sql .= "LEFT JOIN `{$this->App->prefix()}user_tuijian` AS tb3 ON tb3.parent_uid = tb2.uid ";       
        $sql .= "WHERE tb3.id = tb0.user_id AND tb3.uid IS NOT NULL  LIMIT 1";  
        $rt['mymoney3'] = $this->App->findvar($sql);        
        
        if ( ! defined( NAVNAME ) ) define( 'NAVNAME', "分销会员详情" );
        $this->set( 'rt', $rt );
        
        $mb = $GLOBALS['LANG']['mubanid'] > 0 ? $GLOBALS['LANG']['mubanid'] : '';
        $this->template($mb.'/v2_myuserinfo_all');      
    }   

    public function monrydeial_user()
    {
        $this->checked_login();
        ...
        $this->set( 'rt', $rt );
        $mb = $GLOBALS['LANG']['mubanid'] > 0 ? $GLOBALS['LANG']['mubanid'] : '';
        $this->template($mb.'/v2_monrydeial_user');
    }       
    
    public function monrydeial()
    {
        $this->checked_login();
        $this->title("收入明细".' - '.$GLOBALS['LANG']['site_name']);
        //删除
     ...
     * 根据状态获取不同的sql
     */
    public function _return_statue_where( $id = "" ) 
    {
        if ( empty( $id ) ) return "";
        switch ( $id ) {
            case 'weifu':
            ...
            /* 未收货订单 */
            case 'not_received':
                return "tb2.pay_status='1' AND tb2.order_status = '2' AND tb2.shipping_status = '2'";
                break;
            /* 未收货团购订单 */
            case 'not_received_group':
                return "tb2.pay_status='1' AND tb2.order_status = '2' AND tb2.shipping_status = '2'";
                break;
            case 'yifu':
     ...
     * 团购佣金明细
     */
    public function group_money_data()
    {
        $this->title("佣金明细".' - '.$GLOBALS['LANG']['site_name']);
        $uid = $this->Session->read('User.uid');
        $w_rt   = array();
        ...
        $sql = "SELECT SUM(tb1.money) FROM `{$this->App->prefix()}user_money_change_cache` AS tb1 LEFT JOIN `{$this->App->prefix()}group_goods_order_info` AS tb2 ON tb2.order_sn = tb1.order_sn $w";
        $rt['zmoney'] = $this->App->findvar( $sql );
        // 分 页
        $page = isset( $_GET['page'] ) && intval( $_GET['page'] ) > 0 ? intval( $_GET['page'] ) : 1;
        if ( empty( $page ) ) {
        ...
        $list  = 10 ; //每页显示多少个
        $start = ( $page - 1 ) * $list;
        $sql = "SELECT COUNT(tb1.cid) FROM `{$this->App->prefix()}user_money_change_cache` AS tb1 LEFT JOIN `{$this->App->prefix()}group_goods_order_info` AS tb2 ON tb2.order_sn = tb1.order_sn $w";
        $tt = $this->App->findvar( $sql );
        $rt['pages'] = Import::basic()->getpage( $tt, $list, $page, '?page=', true );
        ...
        $sql = "SELECT
                ...
                FROM
                    `{$this->App->prefix()}user_money_change_cache` AS tb1
                LEFT JOIN `{$this->App->prefix()}group_goods_order_info` AS tb2 ON tb2.order_sn = tb1.order_sn
                LEFT JOIN `{$this->App->prefix()}user` AS tb3 ON tb1.buyuid = tb3.user_id $w
                ORDER BY
                    tb1.time DESC
                LIMIT $start, $list";
    ...
    public function _return_goods_name( $sn ) {
        if ( empty($sn) ) return "";
    ...
    public function postmoney()
    {
        ...
        $sql = "SELECT * FROM `{$this->App->prefix()}commission` WHERE user_id = '$uid' LIMIT 1";
        $allmoney = $this->App->findrow( $sql );
        $wallet_list = Common::wallet_list();
        
        if ( ! defined( NAVNAME ) ) define('NAVNAME', "申请提款");
        $this->set( 'rts', $rts );
        $this->set( 'allmoney', $allmoney );
    ...
    // 执行 申请提款
    public function ajax_postmoney( $data = array() )
    {
        $uid = $this->Session->read( 'User.uid' );
        $money     = $data['money'];
        $ids       = $data['id'];
        ...
        /* 2016.7.1 客户要求 星期二才可以提现 */
        if ( date( 'w' ) != 2 )
        {
            $this->jump( ADMIN_URL.'user.php?act=dailicenter', 0, '申请提款请在星期二进行' );
        }
        /* 检查收款账号是否设置 */
        $sql = "SELECT id FROM `{$this->App->prefix()}user_bank` WHERE uid = '$uid' LIMIT 1";
        ...
        if ( $id > 0 )
        {
            ...
            $sql = "SELECT {$field} FROM `{$this->App->prefix()}commission` WHERE user_id = '$uid' LIMIT 1";
            $wallet_money = $this->App->findvar( $sql );
            ...
            /* 检查是否需要众筹 */
            $this->_check_crowd( $uid );
            if ( ! (intval( $ids ) > 0 ) ) $ids = $id;
            $sql = "SELECT * FROM `{$this->App->prefix()}user_bank` WHERE id = '$ids' LIMIT 1";
            $bank_info = $this->App->findrow( $sql );
            
            $dd = array();
            $dd['uid']       = $uid;
            $dd['money']     = $money;
            $dd['addtime']   = time();
            $dd['date']      = date( 'Y-m', time() );
            $dd['bankname']  = $bank_info['bankname'];
            $dd['uname']     = $bank_info['uname'];
            $dd['banksn']    = $bank_info['banksn'];
            $dd['wallet_id'] = $wallet_id;
            $dd['mobile']    = $mobile;
            unset( $bank_info );
            if ( $this->App->insert( 'user_drawmoney', $dd ) )
            {
                $sql = "UPDATE `{$this->App->prefix()}commission` SET $field = $field - {$money} WHERE user_id = '$uid' LIMIT 1";
                $this->App->query( $sql );
                exit( '提款成功，等待我们的处理！' );
            }
            else
     ...
     * 检查是否需要众筹
     * 1、计算已经提现成功的总金额 
         * 2、获取提现众筹额度  $GLOBALS['LANG']['crowd_money']
     * 3、除以额度获得满足几次额度
     * 4、获取支付成功的众筹订单数量
     * 5、满足额度次数-众筹订单数量 > 0 嘿嘿，小子众筹去吧。
     * @author Yusure  http://yusure.cn
     * @date   2016-05-26
     * @param  [param]
     * @return [type]     [description]
     */
    private function _check_crowd( $uid )
    {
        /* 计算已经提现成功的总金额  */
        $condition = "uid = {$uid}";
        $sql = "SELECT SUM(money) FROM `{$this->App->prefix()}user_drawmoney` WHERE {$condition}";
        $money = $this->App->findvar($sql);
        /* 获得满足几次额度 */
        $need_crowd_num = intval($money / $GLOBALS['LANG']['crowd_money']);
        /* 获取支付成功的众筹订单数量 */
        $order_con = "user_id = {$uid} AND pay_status = 1";
        $sql = "SELECT COUNT(*) FROM `{$this->App->prefix()}crowd_goods_order_info` WHERE {$order_con}";
        $crowd_order_num = $this->App->findvar($sql);
        // $this->jump( '跳到商品链接' );
        $need_crowd_num - $crowd_order_num > 0 && $this->jump( 'crowd_catalog.php', 0, '到达提现额度，需要购买众筹商品之后才能提现。' );
    }

    /**
     * 所有佣金   2017.1 引入多种佣金概念 如蛋糕佣金、牛奶佣金
     */
    public function allmoney()
    {
        $this->checked_login();
        $this->title("所有佣金".' - '.$GLOBALS['LANG']['site_name']);
        $uid = $this->Session->read( 'User.uid' );


        $sql = "SELECT * FROM `{$this->App->prefix()}wallet` WHERE state = 1";
        $wallet = $this->App->find( $sql );
        $sql = "SELECT * FROM `{$this->App->prefix()}commission` WHERE user_id = '$uid' LIMIT 1";
        $allmoney = $this->App->findrow( $sql );
        
        if ( ! defined( NAVNAME ) ) define( 'NAVNAME', '所有佣金' );
        $this->set( 'wallet', $wallet );
        $this->set( 'allmoney', $allmoney );
        $mb = $GLOBALS['LANG']['mubanid'] > 0 ? $GLOBALS['LANG']['mubanid'] : '';
        $this->template( $mb . '/v2_allmoney' );
    }
        
    public function postmoneydata( $data = array() )
    {
        $uid = $this->Session->read('User.uid');
    ...
    //重设密码
    public function ajax_rp_pass($data=array()){
        $uname = $data['uname'];
        $email = $data['email'];
        $pass = $data['pass'];
    ...
    //当前文章的分类的所有文章
    public function __get_all_article($type='default'){
    ...
    public function index(){ 
        ...
        if(!defined(NAVNAME)) define('NAVNAME', "代理中心");
        $this->set('rt',$rt);
        $this->template('index');
    }
    
    //AJAX获取我的用户
    public function ajax_myuser_page($rts=array()){
        $hh = $rts['hh'];
        $tops = $rts['tops'];
        $level = $rts['level'];
        $tops = intval($tops);
    ...
    //一级用户
    public function get_myuser_level_1( $uid='0', $start='0', $list='30' ) {
        $sql = "SELECT tb1.*,tb2.subscribe_time,tb2.reg_time,tb2.nickname,tb2.headimgurl,tb2.money_ucount,tb2.points_ucount,tb2.share_ucount,tb2.guanzhu_ucount,tb2.is_subscribe FROM `{$this->App->prefix()}user_tuijian` AS tb1";
        $sql .=" LEFT JOIN `{$this->App->prefix()}user` AS tb2 ON tb1.uid = tb2.user_id";
        $sql .=" WHERE tb1.parent_uid = '$uid' AND tb2.user_rank <> '1' ORDER BY tb2.share_ucount DESC,tb2.money_ucount DESC,tb1.id DESC LIMIT $start,$list";
        return $this->App->find($sql);
    }
    
    //二级用户
    public function get_myuser_level_2( $uid='0', $start='0', $list='30' ) {
    ...
    //三级用户
    public function get_myuser_level_3( $uid='0', $start='0', $list='30' ) {
     ...
     * 用户信息
     */
    public function myuserinfo( $data = array() )
    {
        $this->checked_login();
        $uid = $data['uid'];
        ...
        $sql  = "SELECT SUM(mymoney) FROM `{$this->App->prefix()}user` AS tb0 ";
        $sql .= "LEFT JOIN `{$this->App->prefix()}user_tuijian` AS tb1 ON tb1.uid = tb0.user_id ";
        $sql .= "WHERE tb1.parent_uid='$uid' LIMIT 1";
        ...
        defined( NAVNAME ) or define( 'NAVNAME', '分销会员详情' );
        $this->set( 'rt', $rt );
        $mb = $GLOBALS['LANG']['mubanid'] > 0 ? $GLOBALS['LANG']['mubanid'] : '';
    ...
    public function myuser() {
        ...
        $l = $ts=='1' ? '我的一级代理' : ($ts=='2' ? '我的二级代理' : '我的三级代理');
        ...
        foreach ( (array)$rt['lists'] as $k => $list_v )
        {
            // 一级邀请人数
            $rt['lists'][$k]['zcount1'] = $this->App->findvar("SELECT COUNT(id) FROM `{$this->App->prefix()}user_tuijian` WHERE parent_uid = '$list_v[uid]' LIMIT 1");
        }
        
        if ( $_SERVER['REMOTE_ADDR'] == '221.215.107.170' )
        {
            // echo '<pre>';
            // var_dump( $rt['lists'] );
            // echo '<pre>';
    ...
    public function myusertype() {
        ...
        $zcount1_sql = "SELECT
                            COUNT(id)
                        FROM
                            `{$this->App->prefix()}user_tuijian` AS u_t
                        LEFT JOIN
                            `{$this->App->prefix()}user` AS u
                        ON
                            u_t.uid = u.user_id
                        WHERE
                            u_t.parent_uid = '$uid' AND u.user_rank <> '1'
                        LIMIT 1";
        $rt['zcount1'] = $this->App->findvar( $zcount1_sql );
        //二级
        $sql = "SELECT COUNT(tb1.id) FROM `{$this->App->prefix()}user_tuijian` AS tb1";
        $sql .= " LEFT JOIN `{$this->App->prefix()}user_tuijian` AS tb2 ON tb2.parent_uid = tb1.uid ";
        $sql .= " WHERE tb1.parent_uid='$uid' AND tb2.uid IS NOT NULL  LIMIT 1";
    ...
    public function ajax_getorderlist($data=array()){
    ...
    ###########################
    //用户订单列表
    public function __order_list($w_rt=array(),$page=1,$list=5){
        if(is_array($w_rt)){
            if(!empty($w_rt)){
                $w = " WHERE ".implode(' AND ',$w_rt);
            }
    ...
    public function __order_list_count($w_rt=array()){
        if(is_array($w_rt)){
            if(!empty($w_rt)){
                $w = " WHERE ".implode(' AND ',$w_rt);
            }
    ...
    public function orderinfo($orderid=""){
     ...
     //选择订单的所在状态
     public function select_statue($id=""){
            if(empty($id)) return "";
            switch ($id){
                case '-1':
                    return "";
    ...
    //用户订单操作
    public function ajax_order_op($id=0,$op=""){
   ...
   //我的余额
   public function mymoney($page=1){
   ...
   //ajax删除收藏
   public function ajax_delmycoll($ids=0){
...
*/      
   ...
   public function ajax_feedback($data=array()){
        $err = 0;
   ...
   //删除提问
   public function ajax_delmessages($id=0){
        if(!($id>0))  die("传送的ID为空！");
    ...
    //删除评论
   public function ajax_delcomment($id=0){
        if(!($id>0))  die("传送的ID为空！");
   ...
   //用户积分获取
   public function add_user_jifen($type="",$obj=array()){
    ...
    //更新密码
   public function ajax_updatepass($data= array()){
        $json = Import::json();
        $uid = $this->Session->read('User.uid');
        if(empty($uid)){
    ...
    //注册成功提示的页面
    public function user_regsuccess_mes(){
    ...
    //ajax登录
    public function ajax_user_login($data=array()){
    ...
    //ajax注册
    public function ajax_user_register($data= array()){
        $json = Import::json();
        $result = array('error' => 2, 'message' => '传送的数据为空!');
        if(empty($data['fromAttr']))  die($json->encode($result));
...
/*      $yyy = $fromAttr->yyy;
    ...
    //ajax删除用户收货地址
    public function ajax_delress($id=0){
        $uid = $this->Session->read('User.uid');
    ...
    //设置为默认收货地址
    /*public function ajax_setaddress($data=array()){
        $uid = $this->Session->read('User.uid');
        if(empty($uid)) die("请你先登录！");
    ...
    public function ajax_updateinfo($data= array()){
        $json = Import::json();
        $uid = $this->Session->read('User.uid');
        if(empty($uid)){
            $result = array('error' => 3, 'message' => '先你先登录!');
        ...
        if($uk=='1'){
            $datas['user_rank'] = '1';
            $result = array('error' => 4, 'message' => '你必须购买后才能开通店铺哦！');
            die($json->encode($result));
        }
        $datas['is_salesmen'] = '1';
        ...
        $result = array('error' => 0, 'message' => '你的信息已经提交，审核通过后你将获得代理权限!');
        die($json->encode($result));
   ...
   public function ajax_get_ge_peisong($data=array()){
   ...
   public function ajax_ressinfoop($data=array()){
            $uid = $this->Session->read('User.uid');
...
########################################    
     ...
     * @$width,$height:不设置会自动
     */
    public function vCode($num=4,$size=18, $width=0,$height=0){
