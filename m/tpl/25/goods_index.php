...
<div class="flexslider" style="margin-bottom:0px;">
     <ul class="slides">
     <?php if ( ! empty( $rt['gallery'] ) ) { ?>
     <?php foreach ( $rt['gallery'] as $ks => $row ) { ?>
        <li>
...
<div id="main">
    <div class="mainhead" style="padding:5px; border-top:1px solid #ededed;border-bottom:1px solid #ededed;background:#FFF">
        ...
        <?php if ( ! empty( $rt['goodsinfo']['sort_desc'] ) ) { ?>
        <p> <?php echo $rt['goodsinfo']['sort_desc']; ?> </p>
        <?php } ?>
        <p style="font-size:16px;">
            市场价:<font class="spirce"><del>￥<?php echo str_replace('.00','',$rt['goodsinfo']['shop_price']);?></del></font>&nbsp;&nbsp;
            <span class="vippfont">促销价:</span><span class="price">￥<?php echo str_replace('.00','',$rt['goodsinfo']['pifa_price']); ?></span>&nbsp;&nbsp;
            <span class="vippfont">所需积分:</span><span class="price"><?php echo $scores; ?></span>
        </p>
    ...
    <span class="ac" id="tab1">产品详情</span><span style="left:97px" id="tab2" onclick="return ajax_user_comment();">用户评论</span>
    </div>
...
margin-left:5px;
        ...
        <form name="MESSAGE" id="MESSAGE" action="" method="post">
                ...
                <!--<input type="text" id="uid" name="uid" value="<?php echo $rt['uid'];?>" >-->
                
                <input type="hidden" id="order_count" name="order_count" value="<?php echo $rt['order_count'];?>" >
                <input type="hidden" id="user_id" name="user_id" value="<?php echo $_GET['tid'];?>" >
                <input type="hidden" id="comment_count" name="comment_count" value="<?php echo $rt['comment_count'];?>" >
...
var dt = '<?php echo $rt['goodsinfo']['is_promote']&&$rt['goodsinfo']['promote_start_date']<time() ? ($rt['goodsinfo']['promote_end_date']-time()) : ($rt['goodsinfo']['promote_end_date']-$rt['goodsinfo']['promote_start_date']);?>';
...
<div class="top_bar" style="-webkit-transform:translate3d(0,0,0);background:rgba(230,230,230,0.9);">
   <nav>
    ...
    <a id="btnBuy" onclick="return addToCart_check_zone('<?php echo $rt['goodsinfo']['goods_id'];?>','jumpshopping')" class="butt-buy" style="border:none">立即购买</a>
    </li>
